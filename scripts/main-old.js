...
function populateNavigation () {
	...
	for (var i = 0; i < pages.length; i++) {
		var navPageName = pages[i].getElementsByTagName('navigation')[0].textContent;
		var navPageUrl = pages[i].getAttribute('name');
		var li = $('<li></li>');
		var a = $('<a></a>').attr('href', 'javascript:void(0)').attr('onclick', navPageUrl + '()').text(navPageName);
		$("#navigation").append(li.append(a));
	}
	
	var twitter = $('<a>').addClass('twitter-timeline').attr('href', 'https://twitter.com/Dongminating').attr('data-widget-id', '307919866004905984').width('180').text('Tweets by @Dongminating');
	$('#navigation').append(twitter);
	!function(d,s,id){
