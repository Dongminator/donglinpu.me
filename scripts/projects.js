...
$(document).ready(function(){
	...
	if (proj_id != "") { 
		...
		if ( order > -1 ) {
			...
			$('#proj-nav').children().children()[order].className += " active"
		} else {
...
function populateLeftNav () {
	for (var i = 0; i < projects.length; i++) {
		...
		var a = $('<a></a>').attr('href', '/projects?proj=' + project_id).attr("class","list-group-item").text(project_alias);
...
function displayProject (order) {
	...
	for (var i = 0; i < proj_descriptions.length; i++) {
		...
		$('#proj-main').append($('<p></p>').append(paragraph));
	}
	
	
	var proj_photos = project.getElementsByTagName('description')[0].getElementsByTagName('image');
	
	var fancyBoxPara = $('<p></p>').attr('id','fancyBoxPara').css('display', 'block').appendTo('#proj-main');
	
	
	for (var i = 0; i < proj_photos.length; i++) {
		var proj_image_url = proj_photos[i].textContent;
		
		$('<a></a>').addClass('fancybox-thumb').attr('rel','3yp').attr('href',proj_image_url).append( $('<img></img>').attr('src', proj_image_url).attr('alt',"").css('width',parseInt($('#proj-main').width())/proj_photos.length + "px").css('border', '1px solid white') ).appendTo('#fancyBoxPara');
	}
...
function appendH1 (proj_title, proj_url, proj_id) {
	var h1 = $('<h1></h1>').append( $('<a></a>').attr('href', proj_url).attr('target', '_blank').attr('id', proj_id).text(proj_title) ).css('display','inline-block').appendTo('#proj-main'); // Make it inline so the width of h1 is the width of text
	
	// adjuest h1 width. So it does not exceed one line. 
	var hi1width = h1.width();
	var nyuMainWidth = $('#proj-main').width();
